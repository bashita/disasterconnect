...
def normalize_volunteer_table_schema(conn):
    with conn.cursor() as cursor:
        ...
        if 'age' not in columns:
            cursor.execute("ALTER TABLE volunteers ADD COLUMN age INT")
        if 'gender' not in columns:
            cursor.execute("ALTER TABLE volunteers ADD COLUMN gender VARCHAR(20)")
    conn.commit()
...
def init_db():
    """Connect directly to the existing Aiven database and ensure required tables exist."""
    global DB_AVAILABLE
    ...
    try:
        ...
        # Connect through get_connection() so Aiven SSL/CA settings are used.
        # The Aiven database already exists, so do not issue CREATE DATABASE.
        db = get_connection()
        ...
        print("========== MYSQL INITIALIZATION SUCCESS ==========")
        print("DB_AVAILABLE =", DB_AVAILABLE)
        print("===================================================")
...
def register():
    if request.method == 'POST':
        ...
        conn = None
        try:
            print("Connecting to Aiven database for volunteer registration...")
            conn = get_connection()
